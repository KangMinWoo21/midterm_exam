...
class Explosion(pygame.sprite.Sprite):
    def __init__(self, explodedThing, linger=30):
        ...
        self.image, self.rect = load_image('C:/Users/proje/Downloads/shooting-game-2.0/shooting-game-2.0/data/explosion.png', -1)
...
class Missile(pygame.sprite.Sprite):
    def __init__(self, ship):
        pygame.sprite.Sprite.__init__(self)
        self.image, self.rect = load_image('C:/Users/proje/Downloads/shooting-game-2.0/shooting-game-2.0/data/missile.png', -1)
...
class Powerup(pygame.sprite.Sprite):
    def __init__(self, kindof):
        pygame.sprite.Sprite.__init__(self)
        self.image, self.rect = load_image(kindof + 'C:/Users/proje/Downloads/shooting-game-2.0/shooting-game-2.0/data/_powerup.png', -1)
...
class Ship(pygame.sprite.Sprite):
    def __init__(self, MisType=Missile, ExplosionType=Explosion):
        pygame.sprite.Sprite.__init__(self)
        self.image, self.rect = load_image('C:/Users/proje/Downloads/shooting-game-2.0/shooting-game-2.0/data/ship.png', -1)
        self.original = self.image
        self.shield, self.rect = load_image('C:/Users/proje/Downloads/shooting-game-2.0/shooting-game-2.0/data/ship_shield.png', -1)
...
#우왕이
class CBNU(pygame.sprite.Sprite):
    def __init__(self, color):
        pygame.sprite.Sprite.__init__(self)
        self.loc = 0
        self.image, self.rect = load_image('C:/Users/proje/Downloads/shooting-game-2.0/shooting-game-2.0/data/space_invader_' + color + '.png', -1)
        screen = pygame.display.get_surface()
        self.area = screen.get_rect()
        self.rect.midtop = (random.randint(
            self.area.left + self.rect.width // 2,
...
class Spikey(CBNU):
    def __init__(self):
        CBNU.__init__(self, 'blue')
        self.slope = random.choice(list(x for x in range(-3, 3) if x != 0))
        self.period = random.choice(list(4 * x for x in range(10, 41)))
        self.moveFunc = lambda: (self.slope * (
                    self.loc % self.period) if self.loc % self.period < self.period // 2 else self.slope * self.period // 2 - self.slope * (
                    (self.loc % self.period) - self.period // 2), 0)


class Fasty(CBNU):
    def __init__(self):
        CBNU.__init__(self, 'white')
...
def main():
    ...
    ship = Ship(Missile)
    CBNUTypes = (Siney, Spikey, Roundy, Fasty)
    powerupTypes = (BombPowerup, ShieldPowerup)

    CBNUs = pygame.sprite.Group()
    ...
    CBNUPeriod = 50
    curTime = 0
    CBNUsThisWave, CBNUsLeft, CBNUsOffScreen = 10, 10, 10
    wave = 1
    bombsHeld = 3
    score = 0
    ...
    while ship.alive:
        ...
        for CBNU in CBNUs:
            if CBNU.rect.top > CBNU.area.bottom:
                CBNU.kill()
                CBNUsOffScreen += 1
            for bomb in bombs:
                if pygame.sprite.collide_circle(bomb, CBNU):
                    CBNU.explode().add(allsprites, explosions)
                    CBNUsLeft -= 1
                    score += 1
            for missile in missiles:
                if pygame.sprite.collide_rect(missile, CBNU):
                    CBNU.explode().add(allsprites, explosions)
                    missile.kill()
                    CBNUsLeft -= 1
                    score += 1
            if pygame.sprite.collide_rect(CBNU, ship):
                if ship.shieldUp:
                    ship.shieldUp = False
                    CBNU.explode().add(allsprites, explosions)
                    CBNUsLeft -= 1
        ...
        # Update CBNUs 수 늘리기
        if curTime <= 0 and CBNUsOffScreen > 0:
            random.choice(CBNUTypes)().add(CBNUs, allsprites)
            CBNUsOffScreen -= 1
            curTime = CBNUPeriod
        elif curTime > 0:
            curTime -= 1
        ...
        leftText = font.render("CBNU: " + str(CBNUsLeft), 1, (0, 0, 255))
        ...
        if CBNUsLeft <= 0:
            if betweenWaveCount > 0:
                ...
                wave += 1
                CBNUsLeft, CBNUsOffScreen, CBNUsThisWave = 3 * [2 * CBNUsThisWave]
                betweenWaveCount = betweenWaveTime
